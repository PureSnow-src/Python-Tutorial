...
当程序规模变大后，如果所有代码都堆在一起，就会越来越难读、越来越难改、越来越难检查错误。函数与结构化编程的核心目的，就是把一个复杂问题拆成若干个较小、较清晰的部分，让程序的逻辑更加有条理。

## 什么是函数

函数可以理解为“一个可以重复调用的功能块”。  
把一段完成特定任务的代码封装起来，起一个名字，以后需要时直接调用即可。

例如，前面我们写过计算矩形面积，如果以后要重复使用这个功能，写成函数就更合适。
...
并不是每个函数都必须写 `return`。如果一个函数只是执行某种操作，不需要把结果返回给外部，也可以不写返回值。
...
参数的作用是让同一个函数可以处理不同的数据。这样，函数就不再只适用于某一个固定场景，而是能被反复复用。
...
- 能在函数内部定义的变量，尽量不要随意依赖外部变量
- 一个函数尽量只处理与自己任务直接相关的数据
- 这样写出来的函数更清晰，也更容易复用

## 递归

递归是函数的一种特殊用法，即**函数在自己的定义过程中调用自己**。

有些问题本身就适合拆成“和原问题同类型、但规模更小”的子问题，这时就可以考虑使用递归来解决。
...
1. **终止条件**：在某种情况下，不再继续调用自己
2. **递归关系**：把原问题转化为一个规模更小的同类问题

如果没有终止条件，递归就会无限进行下去，最终导致程序报错。
...
这正好符合递归的思想，因为 `n!` 可以转化为更小规模的 `(n-1)!`。
...
递归写法通常比较简洁，但它不一定总是最省资源。对于初学者来说，最重要的是理解它的逻辑结构：**先找到终止条件，再写出“规模缩小”的递推关系**。

## 什么是结构化编程

结构化编程是一种组织程序的思想。它强调：  
**程序应该由清晰的顺序结构、选择结构和循环结构组成，并且把不同任务拆分成相对独立的函数。**

前面学过的三种基本控制结构分别是：

- **顺序结构**：代码从上到下依次执行
- **选择结构**：根据条件执行不同分支，例如 `if...elif...else`
- **循环结构**：重复执行某段代码，例如 `for`、`while`

函数的作用，就是把这些基本结构组合起来，形成更清楚、更容易维护的程序。

## 为什么要进行结构化编程

如果把所有功能都写在一起，程序会出现这些问题：
...
因此，在编写稍微复杂一点的程序时，应该逐步养成“先拆分任务，再分别实现”的习惯。

## 用函数组织一个完整程序

下面以“输入若干成绩，计算平均分并判断是否及格”为例，说明如何把一个程序拆成多个函数。
...
- `input_scores()`：负责输入数据
- `average_list(nums)`：负责计算平均值
- `judge_result(avg)`：负责根据平均分给出结论
- `main()`：负责组织整个程序流程

这样的写法就比“把所有代码全部堆在一起”更清晰，这就是结构化编程最直接的体现。
...
2. 编写函数 `count_positive(nums)`，输入一个列表，返回其中正数的个数。
3. 编写递归函数，计算斐波那契数列的第 `n` 项。斐波那契数列计算公式如下：
...
5. 将“输入 3 个成绩，计算平均分并判断是否及格”写成结构化程序。要求至少拆分为“输入函数”“求平均值函数”“判断函数”三个部分。
