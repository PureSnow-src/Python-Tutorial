...
当我们不知道循环执行多少次，而是“只要条件成立就一直做”时，可以使用 `while`。
...
这里的 `count += 1` 非常重要，否则条件会一直成立，程序就会陷入死循环。
...
2. 输入一个整数，判断它是奇数还是偶数。
3. 用 `for` 循环输出 1 到 100 中所有 3 的倍数。
4. 用 `for` 循环计算 1 到 100 的平方和。
5. 输入 5 个数，输出其中的最大值。
6. 不断输入整数，直到输入 `0` 为止，最后输出这些非零整数的和。
7. 输出九九乘法表。
...
当代码写得更多时，我们通常不会把所有内容都写在一个文件里，而是分开放在不同文件中。一个 `.py` 文件通常就可以看作一个模块。

例如，假设有一个 `my_math.py` 文件：
...
Python 自带了很多标准库。比如 `math` 模块可用于数学运算。
...
这说明：除了自己写函数，我们也可以直接调用别人已经写好的功能。

## if __name__ == "__main__":

当一个 Python 文件既可能被“直接运行”，也可能被“当模块导入”时，常会看到下面这段代码：
...
初学时可以先把它理解为：**只有直接运行这个文件时，`main()` 才会执行**。
...
7. 建立一个 `my_math.py` 文件，在其中定义加法函数和减法函数；再在另一个文件中导入并调用它们。
